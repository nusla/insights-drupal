...
function update_script_selection_form($form, &$form_state) {
	...
	foreach ( update_resolve_dependencies ( $starting_updates ) as $function => $data ) {
		if (! $data ['allowed']) {
			...
			$form ['start'] = array (
					$module_update_key => $form ['start'] [$module_update_key] 
			) + $form ['start'];
		}
	}
	...
	if (empty ( $count )) {
		...
		if ($incompatible_count) {
			$form ['start'] ['#title'] = format_plural ( $count, '1 pending update (@number_applied to be applied, @number_incompatible skipped)', '@count pending updates (@number_applied to be applied, @number_incompatible skipped)', array (
 ...
 *
 * @return Rendered HTML warning with 403 status.
 */
 ...
 *
 * @return TRUE if the current user should be granted access, or FALSE otherwise.
 */
 ...
 *
 * @param $skip_warnings (optional)
 *        	If set to TRUE, requirement warnings will be ignored, and a
	...
	switch ($op) {
		...
		case 'Apply pending updates' :
			if (isset ( $_GET ['token'] ) && drupal_valid_token ( $_GET ['token'], 'update' )) {
				...
				$redirect_url = $base_root . drupal_current_script_url ( array (
						'op' => 'results' 
				) );
